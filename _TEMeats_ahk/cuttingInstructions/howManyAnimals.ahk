/*
    ...
    Parameters:
    - origin: An object containing information about the animal (e.g., animal type, count).
...
howManyAnimals(origin?)
{
    ...
    gui_.howMany.Assign := gui_.Add("Checkbox", "", "Assign to Slaughtered Animals")
    GuiResizer.FormatOpt(gui_.howMany.Assign, bx + 0.14 + 0.2, by, 0.24, bh)
    ...
    gui_.howMany.Header.Opt("Background272727")
    gui_.howMany.AnimalText.Opt("Background272727")
    ...
    ContinueToCutsheet(*)
    {
        ...
        if gui_.howMany.Edit.Value != 0 or gui_.howMany.Edit.Value != "0"
        {
            ...
            if gui_.howMany.Assign.Value
            {
                if IsObject(AssignAnimalsCS.getAnimals())
                    myGui := AssignAnimalsCS()
                    myGui.Show("w1162 h843")
                    darkmode(myGui)
                    while !AssignAnimalsCS.finished
                        Sleep(10)
                    GuiFromHWND(AssignAnimalsCS.hwnd).Destroy()
            }
