...
Class GuiReSizer
{
    ...
    static FormatOpt(ctrl, xp?, yp?, wp?, hp?, anchor?) 
    {
        ...
        if IsSet(xp)
        {
            if InStr(xp, "+")
            {
                numx := Number(Trim(StrReplace(xp, "+", "")))
                xp := GuiReSizer.lastX += numx
            } else if InStr(xp, "-") {
                numx := Number(Trim(StrReplace(xp, "-", "")))
                xp := GuiReSizer.lastX -= numx
            }
        ...
        if IsSet(yp)
        {
            if InStr(yp, "+")
            {
                numy := Number(Trim(StrReplace(yp, "+", "")))
                yp := GuiReSizer.lastY += numy
            } else if InStr(yp, "-") {
                numy := Number(Trim(StrReplace(yp, "-", "")))
                yp := GuiReSizer.lastY -= numy
            }
        ...
        if IsSet(wp) {
            if InStr(wp, "+")
            {
                numw := Number(Trim(StrReplace(wp, "+", "")))
                wp := GuiReSizer.lastW += numw
            } else if InStr(wp, "-") {
                numw := Number(Trim(StrReplace(wp, "-", "")))
                wp := GuiReSizer.lastW -= numw
            }
        ...
        if IsSet(hp) {
            if InStr(hp, "+")
            {
                numh := Number(Trim(StrReplace(hp, "+", "")))
                hp := GuiReSizer.lastH += numh
            } else if InStr(hp, "-") {
                numh := Number(Trim(StrReplace(hp, "-", "")))
                hp := GuiReSizer.lastH -= numh
            }
