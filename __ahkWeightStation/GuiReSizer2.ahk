...
Class GuiReSizer
{
    ...
    Static Add(GuiObj, CtrlType, sizePos, LabelVar)
    {
        ...
        if (matchCount)
        {
            ...
            Loop count
            { 
                for letter in lettersToLookFor
                {
                    if InStr(letter, matchObj[index])
                    {
                        percentage := Integer(StrReplace(matchObj[index+1], "%", ""))
                        returnCtrlVar.%letter%P := percentage
                    }
                }
