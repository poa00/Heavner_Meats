...
switchCutsheetCRUDToWizard()
{
    ...
    gui_.Wiz.main.Header3.Value := "" Customer.producer['full_name'] ": `n" Customer.event.animals.total " " Customer.event.animals.animal
    gui_.Wiz.main.Header3.Value .= "`n" Customer.event.animals.temp_remaining " " Customer.event.animals.animal " remaining"
...
loadSavedCutsheet(setPrimaryCheckbox?)
{
    ...
    if IsSet(setPrimaryCheckbox) 
    {
        ...
        {
            if IsObject(arr) && arr.Has("details")
            {
                t := Type(arr['details'])
                if t = "Array" {
                    if arr.Has('details') && arr['details'].HasProp('Length') && arr['details'].Length != 0
                        gui_.Primary.LV.SetCell(A_Index, 2, "✓")
                }
                else if t = "Map"
                    if arr['details'].Has("split") || arr['details'].Has("split")
                        gui_.Primary.LV.SetCell(A_Index, 2, "✓")
            }
        }
    }
